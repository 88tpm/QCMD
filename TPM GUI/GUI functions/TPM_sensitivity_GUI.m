function [output] = TPM_sensitivity_GUI(controls,properties,fixed_properties)
...
temp_properties = properties; % create a holding variable for loop
for x_count = 1:controls.steps+1;
    for y_count = 1:controls.steps+1;
        temp_properties(x_name) = variables(x_count,2);
        temp_properties(y_name) = variables(y_count,1);
        [combs,output.score_surface(x_count,y_count,:),~] = combo_slaved(temp_properties,fixed_properties,controls.f_sensitivity,controls.d_sensitivity,controls.f0,controls.linear);
...
elseif strcmp(controls.harmonic_type,'FD')
    ...
    a = 'TPM-sensitivity plot for F'; b = ' and D';
...
text = [a f b d];
figures = 1;
[figures] = sketch(variables(:,2),variables(:,1),squeeze((output.score_surface(:,:,row)))',text,figures,controls.z_range(1),controls.z_range(2),'g',controls.publication,x_text,y_text);

if controls.text
    [~,~,~] = combo_text(properties,fixed_properties,controls.f_sensitivity,controls.d_sensitivity,controls.f0,controls.linear,controls.text,controls.text_rows);
end

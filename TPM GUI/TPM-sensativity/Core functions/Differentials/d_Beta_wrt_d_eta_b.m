function [result]=d_Beta_wrt_d_eta_b(f1,harmonic_index,inputs)

% Calculates the value of the differential of the Beta function with
% respect to the viscosity of the fluid (eta b).
% 
% 
% Inputs
%   f1
...
result=((-1/2)+1i*(-1/2)).*pi.^(1/2).*(f1.*N.*eta_f.^(-1).*rho_f).^( ...
    1/2).*(cos(2.*f1.*h1.*N.*pi.*((2i).*f1.*N.*pi.*eta_1+ ...
    mu_1).^(-1/2).*rho_1.^(1/2))+((-1)+1i).*f1.*N.*pi.^(1/2).*eta_f.* ...
    rho_f.*(f1.*N.*eta_f.*((2i).*f1.*N.*pi.*eta_1+mu_1).*rho_1.*rho_f).^( ...
    -1/2).*sin(2.*f1.*h1.*N.*pi.*((2i).*f1.*N.*pi.*eta_1+ ...
    mu_1).^(-1/2).*rho_1.^(1/2))).^(-2);